...
class Example(MetricsExampleBase):
    ...
    def __init__(self, example_id, output_size, skip_first_cnt):
        ...
        self._acc = MetricSingleLabelAccuracy(dim=output_size)
        self._loss = MetricSum(dim=output_size)
    ...
    def _append(self, output, labels):
        ...
        # Handle both regression (2D y) and classification (1D y)
        try:
            labels = labels[[-1], :]
        except IndexError:
            labels = labels[[-1]]
    ...
    def stats(self):
        loss_last = self._loss.mean_x().flatten()[self.true_label]
        vote_acc_last = self._acc.vote_accuracy()
        vote_prob_last = self._acc.vote_probability()
        log_acc_last = self._acc.log_accuracy()
        log_prob_last = self._acc.log_probability()
...
class MetricsMinimalClassification(MetricsBase):
    """
